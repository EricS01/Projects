...
public class APIRecipeController extends APIController {
     ...
     * to update an Recipe by automatically converting the JSON RequestBody
     * provided to a Recipe object. Invalid JSON will fail.
     *
     * @param recipe
     *            The valid Recipe to be saved.
    ...
    public ResponseEntity editRecipe ( @RequestBody final Recipe recipe ) {
        for ( final Ingredient i : recipe.getIngredients() ) {
            if ( i.getName() == null || i.getName().length() == 0 ) {
                return new ResponseEntity( errorResponse( "Ingredient must have name." ), HttpStatus.BAD_REQUEST );
            }
            if ( i.getAmount() < 0 ) {
