...
public class APIInventoryController extends APIController {
    ...
    public ResponseEntity updateInventory ( @RequestBody final Inventory inventory ) {
        ...
        final boolean addable = inventoryCurrent.addIngredients( inventory.getIngredients() );
        if ( !addable ) {
            return new ResponseEntity( errorResponse( "Ingredient amount must be a positive integer." ),
                    HttpStatus.BAD_REQUEST );
        }
        service.save( inventoryCurrent );
        return new ResponseEntity( successResponse( "Successfully added Ingredients" ), HttpStatus.OK );
    }

    /**
     * REST API endpoint to provide update access to CoffeeMaker's singleton
     * Inventory. This will update the Inventory of the CoffeeMaker by adding a
     * new Ingredient provided to the CoffeeMaker's stored inventory
     *
     * @param inventory
     *            amounts to add to inventory
     * @return response to the request
    ...
    public ResponseEntity addNewIngredient ( @RequestBody final Ingredient newIngredient ) {
        final Inventory inventoryCurrent = service.getInventory();
        final boolean ableToAdd = inventoryCurrent.addNewIngredient( newIngredient );
        if ( ableToAdd ) {
            service.save( inventoryCurrent );
            ...
            return new ResponseEntity(
                    errorResponse( "Ingredient with the name " + newIngredient.getName() + " already exists." ),
                    HttpStatus.CONFLICT );
        }
