...
public class APIUserController extends APIController {
    ...
    /** All roles */
    private static final List<String>         ALL_ROLES            = List.of( ROLE_ADMIN, ROLE_PATIENT, ROLE_HCP,
            ROLE_ER, ROLE_LABTECH, ROLE_VIROLOGIST, ROLE_OD, ROLE_OPH, ROLE_VACCINATOR, ROLE_BSM,
     ...
     * @return response
     */
    @PutMapping ( BASE_PATH + "/users/{id}/adv" )
    public ResponseEntity updateAdvocate ( @PathVariable final String id,
            @RequestBody final PatientAdvocateForm userF ) {
        User user = null;
        final List<Role> rolesOnUser = userF.getRoles().stream().map( Role::valueOf ).collect( Collectors.toList() );
        try {
            if ( rolesOnUser.contains( Role.ROLE_PATIENT ) ) {
    ...
    public ResponseEntity deleteUser ( @PathVariable final String id ) {
        ...
        try {
            ...
            // If we are looking at a PatientAdvocate, delete all the
            // associations first to prevent a foreign key error
            if ( user.getRoles().contains( Role.ROLE_PATIENTADVOCATE ) ) {
                final PatientAdvocate pa = (PatientAdvocate) user;
                for ( final PatientAdvocateAssociation a : pa.getAssociatedPatients() ) {
                    a.getPatient().removeAssociatedPatientAdvocates( a );
                    userService.save( a.getPatient() );
                    patientAdvocateAssociationService.delete( a );
                }
            }
    ...
    public ResponseEntity generateUsers () {
        ...
        final User patientAdvocate = new PatientAdvocate(
                new UserForm( "patientadvocate", "123456", Role.ROLE_PATIENTADVOCATE, 1 ) );

        userService.save( patient );

        userService.save( patientAdvocate );
