...
public class APIOfficeVisitController extends APIController {
    ...
    /**
     * Retrieves a list of all the Office Visits in the database for the user
     *
     * @param username
     *            Username of the patient
     * @return list of office visits
     */
    @GetMapping ( BASE_PATH + "/officevisits/patientadvocate/{username}" )
    @PreAuthorize ( "hasAnyRole('ROLE_PATIENTADVOCATE')" )
    public List<OfficeVisit> getOfficeVisitsPatientAdvocate ( @PathVariable final String username ) {
        final User patient = userService.findByName( username );

        if ( patientAdvocateAssociationService.findByUsernames( username, LoggerUtil.currentUser() ) == null ) {
            return null;
        }

        loggerUtil.log( TransactionType.VIEW_ALL_OFFICE_VISITS, username );
