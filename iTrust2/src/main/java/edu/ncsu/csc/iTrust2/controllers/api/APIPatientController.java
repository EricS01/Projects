...
public class APIPatientController extends APIController {
    ...
    /**
     * Get a list of all the PatientAdvocates associated with a Patient. The
     * provided ID is the ID of a Patient that is in the system.
     *
     * @param id
     *            of the patient
     * @return response
     */
    @GetMapping ( BASE_PATH + "/patients/associated/{id}" )
    @PreAuthorize ( "hasAnyRole('ROLE_PATIENTADVOCATE', 'ROLE_ADMIN')" )
    public ResponseEntity getAssociatedPatientAdvocates ( @PathVariable ( "id" ) final String id ) {
        ...
        return new ResponseEntity( patient.getAssociatedPatientAdvocates(), HttpStatus.OK );
    }

    /**
     * Get a list of all the PatientAdvocates associated with a Patient. The
     * current user is used instead of a passed in id.
     *
     * @return response
     */
    @GetMapping ( BASE_PATH + "/patients/associated" )
    @PreAuthorize ( "hasAnyRole('ROLE_PATIENT', 'ROLE_ADMIN')" )
    public ResponseEntity getAssociatedPatientAdvocates () {
        final Patient patient = (Patient) patientService.findByName( LoggerUtil.currentUser() );
        if ( null == patient ) {
            return new ResponseEntity( errorResponse( "No patient found for id " + LoggerUtil.currentUser() ),
                    HttpStatus.NOT_FOUND );
        }
