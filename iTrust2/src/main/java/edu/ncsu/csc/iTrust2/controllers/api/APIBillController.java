...
public class APIBillController extends APIController {
    ...
    /**
     * Retrieves a list of all the Bills in the database for the user
     *
     * @param username
     *            Username of the patient
     * @return list of bills
     */
    @GetMapping ( BASE_PATH + "/bills/patientadvocate/{username}" )
    @PreAuthorize ( "hasAnyRole('ROLE_PATIENTADVOCATE')" )
    public List<Bill> getBillsPatientAdvocate ( @PathVariable final String username ) {
        final User patient = userService.findByName( username );
