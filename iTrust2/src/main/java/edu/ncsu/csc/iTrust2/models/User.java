...
public class User extends DomainObject {
    ...
    public void setRoles ( final Set<Role> roles ) {
        ...
        if ( roles.contains( Role.ROLE_PATIENTADVOCATE ) && roles.contains( Role.ROLE_HCP ) ) {
            throw new IllegalArgumentException(
                    "Tried to create PatientAdvocate with HCP role. PatientAdvocate cannot also be a HCP!" );
        }
        if ( roles.size() == 0 ) {
            throw new IllegalArgumentException(
