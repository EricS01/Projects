...
function FavoriteCard(
    ...
    const [removeFavorite, setRemoveFavorite] = useState("Remove From Favorites");

    const handleRemoveFavorites = async () => {
        fetch(`/api/favorites/${data.favorite_id}`, {
            method: 'DELETE',
            headers: {
        ...
        >
            ...
            {expanded && ownProfile && <>
                ...
                <div className='mt-2 border-t-2 border-gray-300 flex space-x-4 text-sm justify-evenly'>
                    <button onClick={handleRemoveFavorites}  className='w-1/3 p-2 mt-2 text-white align-middle transition duration-200 rounded-lg shadow-lg bg-tj-light-brown hover:bg-tj-lightish-brown'>
                        {removeFavorite}
                    </button>
                </div>
            </>
